...
int main() {
    ...
    std::cout << "  Conduit Path: " << SHM_PATH << "\n";
    std::cout << "========================================================\n";

    unlink(SHM_PATH);
    int shm_fd = open(SHM_PATH, O_CREAT | O_RDWR, 0666);
    if (shm_fd < 0) {
        std::cerr << "[Fatal Error] Failed to create shared memory file\n";
        return 1;
    }
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        shm->telemetry.timestamp_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
        ...
        if (tick_count % 1000 == 0) {
            std::cout << "[500 Hz Tick: " << tick_count << "] Margin: " 
                      << safety.barrier_margin << " m | Joint 5 Torque: " 
    ...
    std::cout << "\n[Shutdown] Cleaning up shared memory segment...\n";
